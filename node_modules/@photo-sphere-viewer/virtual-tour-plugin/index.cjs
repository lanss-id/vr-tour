...
var CSS2DObject = class extends import_three.Object3D {
  /**
  ...
  constructor(element = document.createElement("div")) {
    ...
    this.center = new import_three.Vector2(0.5, 0.5);
...
var _vector = new import_three.Vector3();
var _viewMatrix = new import_three.Matrix4();
var _viewProjectionMatrix = new import_three.Matrix4();
var _a = new import_three.Vector3();
var _b = new import_three.Vector3();
...
var _position = new import_three2.Vector3();
var _quaternion = new import_three2.Quaternion();
var _scale = new import_three2.Vector3();
var CSS3DObject = class extends import_three2.Object3D {
  /**
...
var _matrix = new import_three2.Matrix4();
var _matrix2 = new import_three2.Matrix4();
...
var ArrowsRenderer = class extends import_core2.AbstractComponent {
  constructor(parent, plugin) {
    ...
    this.camera = this.is3D ? new import_three3.PerspectiveCamera(30, 1) : null;
    this.scene = new import_three3.Scene();
    this.viewer.addEventListener(import_core2.events.ReadyEvent.type, this, { once: true });
    this.viewer.addEventListener(import_core2.events.PositionUpdatedEvent.type, this);
    this.viewer.addEventListener(import_core2.events.SizeUpdatedEvent.type, this);
  ...
  destroy() {
    ...
    this.viewer.removeEventListener(import_core2.events.PositionUpdatedEvent.type, this);
    this.viewer.removeEventListener(import_core2.events.SizeUpdatedEvent.type, this);
  ...
  handleEvent(e) {
    switch (e.type) {
      ...
      case import_core2.events.SizeUpdatedEvent.type:
      case import_core2.events.PositionUpdatedEvent.type:
        this.__updateCamera();
        break;
      case import_core2.events.RenderEvent.type:
        this.render();
        break;
      case import_core2.events.ClickEvent.type: {
      ...
      case "mouseenter": {
        const link = this.__getTargetLink(import_core2.utils.getEventTarget(e));
        if (link) {
      ...
      case "mousemove": {
        const link = this.__getTargetLink(import_core2.utils.getEventTarget(e), true);
        if (link) {
  ...
  __updateCamera() {
    ...
    if (this.is3D) {
      ...
      position.pitch = import_three3.MathUtils.clamp(position.pitch, -this.arrowsPosition.maxPitch, -this.arrowsPosition.minPitch);
  ...
  render() {
    if (this.is3D) {
      ...
      this.scene.children.forEach((object) => {
        ...
        if (data.conflict) {
          const distance2 = Math.abs(import_core2.utils.getShortestArc(position.yaw, data.yaw));
  ...
  addLinkArrow(link, position, depth = 1) {
    ...
    if (conf.className) {
      import_core2.utils.addClasses(element, conf.className);
    }
    if (conf.style) {
    ...
    if (this.is3D) {
      ...
      this.scene.children.forEach((object2) => {
        ...
        if (Math.abs(import_core2.utils.getShortestArc(data.yaw, position.yaw)) < this.arrowsPosition.linkOverlapAngle) {
          data.conflict = true;
          conflict = true;
        }
      });
  ...
  __getTargetLink(target, closest = false) {
    const target2 = closest ? import_core2.utils.getClosest(target, ".psv-virtual-tour-link") : target;
...
var AbstractDatasource = class {
  ...
  checkNode(node) {
    if (!node.id) {
      throw new import_core3.PSVError("No id given for node");
    }
    if (!node.panorama) {
      throw new import_core3.PSVError(`No panorama provided for node ${node.id}`);
    }
    if (this.plugin.isGps && !(node.gps?.length >= 2)) {
      throw new import_core3.PSVError(`No GPS position provided for node ${node.id}`);
    }
    if (!this.plugin.isGps && node.markers?.some((marker) => marker.gps && !marker.position)) {
      throw new import_core3.PSVError(`Cannot use GPS positioning for markers in manual mode`);
    }
    if (!node.links) {
  ...
  checkLink(node, link) {
    if (!link.nodeId) {
      throw new import_core3.PSVError(`Link of node ${node.id} has no target id`);
    }
    if (link.nodeId === node.id) {
      throw new import_core3.PSVError(`Node ${node.id} links to itself`);
    }
    if (!this.plugin.isGps && !import_core3.utils.isExtendedPosition(link.position)) {
      throw new import_core3.PSVError(`No position provided for link ${link.nodeId} of node ${node.id}`);
    }
    if (this.plugin.isGps && !link.gps) {
      throw new import_core3.PSVError(`No GPS position provided for link ${link.nodeId} of node ${node.id}`);
    }
  }
};
...
var ClientSideDatasource = class extends AbstractDatasource {
  async loadNode(nodeId) {
    if (this.nodes[nodeId]) {
      ...
      throw new import_core4.PSVError(`Node ${nodeId} not found`);
    }
  }
  setNodes(rawNodes) {
    if (!rawNodes?.length) {
      throw new import_core4.PSVError("No nodes provided");
    }
    const nodes = {};
    const linkedNodes = {};
    rawNodes.forEach((node) => {
      this.checkNode(node);
      if (nodes[node.id]) {
        throw new import_core4.PSVError(`Duplicate node ${node.id}`);
      }
      nodes[node.id] = node;
    });
    rawNodes.forEach((node) => {
    ...
    rawNodes.forEach((node) => {
      if (!linkedNodes[node.id]) {
        import_core4.utils.logWarn(`Node ${node.id} is never linked to`);
      }
    });
    this.nodes = nodes;
  }
  updateNode(rawNode) {
    ...
    if (!node) {
      throw new import_core4.PSVError(`Node ${rawNode.id} does not exist`);
    }
  ...
  __checkLinks(node, nodes) {
    node.links.forEach((link) => {
      if (!nodes[link.nodeId]) {
        throw new import_core4.PSVError(`Target node ${link.nodeId} of node ${node.id} does not exists`);
      }
...
var ServerSideDatasource = class extends AbstractDatasource {
  constructor(plugin, viewer) {
    ...
    if (!plugin.config.getNode) {
      throw new import_core5.PSVError("Missing getNode() option.");
    }
...
function gpsDegToRad(gps) {
  return [import_three4.MathUtils.degToRad(gps[0]), import_three4.MathUtils.degToRad(gps[1])];
}
function distance(p1, p2) {
  return import_core6.utils.greatArcDistance(p1, p2) * 6371e3;
}
...
var _VirtualTourPlugin = class _VirtualTourPlugin extends import_core7.AbstractConfigurablePlugin {
   ...
   */
  handleEvent(e) {
    if (e instanceof import_core7.events.ClickEvent) {
  ...
  getLinkPosition(nodeId) {
    ...
    if (!link) {
      throw new import_core7.PSVError(`Cannot find link "${nodeId}"`);
    }
  ...
  updateNode(newNode) {
    if (this.isServerSide) {
      throw new import_core7.PSVError("Cannot update node in server side mode");
    }
  ...
  __getGpsMapPosition(gps) {
    ...
    if (this.isGps && map && map.extent && map.size) {
      return {
        x: import_three5.MathUtils.mapLinear(gps[0], map.extent[0], map.extent[2], 0, map.size.width),
        y: import_three5.MathUtils.mapLinear(gps[1], map.extent[1], map.extent[3], 0, map.size.height)
      };
    } else {
      return null;
    }
...
_VirtualTourPlugin.VERSION = "5.13.4";
